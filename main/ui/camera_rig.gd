...
func enable_tabletop_pan(start_pos := self.global_position): #switch to tabletop pan
	pan_mode = true
	tracked_player = null
...
func _input(event):
	...
	if event.is_action_pressed("zoom_in"):
		pan_offset.y = clamp(pan_offset.y - (zoom_speed * zoom_sensitivity * 0.1), 2.0, 9.0)
		pan_offset.z = clamp(pan_offset.z - (zoom_speed * zoom_sensitivity * 0.1), 2.0, 9.0)
