...
func _execute_money_swap(p1, p2):
	var temp_money: int = p2.money * 0.25
	p1.money += temp_money
	p2.money -= temp_money
	gc.ui.update_ui()
...
func handle_draw_card(player, is_chance):
	...
	match card_data["type"]:
		...
		"swap_money":
			var target_idx: int
			if player.is_ai:
				target_idx = _ai_choose_richest_player(player,true)
				print("[AI] Getting 25% money from: " + gc.players[target_idx].player_name)
			else:
				gc.ui.show_target_selector(gc.players, gc.current_player, "Select a player to get money from:")
		...
		"skip_other_turn":
			var target_idx: int
			if player.is_ai:
				target_idx = _ai_choose_richest_player(player,false)
...
func _ai_execute_property_steal(ai_player):
	...
	for t in gc.tiles:
		if not t.tile_type in [BoardData.TileType.PROPERTY, BoardData.TileType.UTILITY, BoardData.TileType.CAFE]:
			continue
		if t.tile_owner == ai_player: continue
		var price = t.tile_data.get("price", 0)

		var score = float(price)

		if t.tile_owner != null and not t.tile_owner.is_bankrupt:
			var t_color = t.tile_data.get("color", "")

			if t.tile_type == BoardData.TileType.PROPERTY and t_color != "":
				var set_size := 0; var ai_has := 0
				for st in gc.tiles:
					if st.tile_type == BoardData.TileType.PROPERTY and st.tile_data.get("color","") == t_color:
						set_size += 1
						if st.tile_owner == ai_player: ai_has += 1
				if ai_has == set_size - 1:
					score += 2000
			var owner = t.tile_owner
			if t_color != "" and gc.board_state.has_monopoly(owner, t_color):
				var monopoly_built := false
				for st in gc.tiles:
					if st.tile_type == BoardData.TileType.PROPERTY and st.tile_data.get("color","") == t_color:
						if st.funding > 0:
							monopoly_built = true
							break
				if not monopoly_built:
					score += 800
			else:
				score += 150

		score += randf_range(0.0, 80.0)

		candidates.append({ "tile": t, "score": score })
	...
	var best_target = candidates[0]["tile"]

	print("[AI] Stealing: ", best_target.tile_data.get("name", "?"),
		  " (score: ", int(candidates[0]["score"]), ")")
...
func _ai_execute_property_swap(ai_player):
	if ai_player.properties.is_empty():
	...
	for t in ai_player.properties:
		if t.tile_type != BoardData.TileType.PROPERTY: continue
		var color = t.tile_data.get("color", "")

		var has_buildings := false
		for ot in gc.tiles:
			if ot.tile_type == BoardData.TileType.PROPERTY and ot.tile_data.get("color","") == color:
				if ot.funding > 0: has_buildings = true; break
		if has_buildings: continue

		var set_size := 0; var ai_has := 0
		for ot in gc.tiles:
			if ot.tile_type == BoardData.TileType.PROPERTY and ot.tile_data.get("color","") == color:
				set_size += 1
				if ot.tile_owner == ai_player: ai_has += 1
		if ai_has >= set_size - 1: continue

		var score = float(t.tile_data.get("price", 0))
		if score < worst_score:
			worst_score = score
			my_worst    = t

	if my_worst == null:
		print("[AI] No safe property to offer in swap.")
		gc.show_default_actions()
		return
	...
	for t in gc.tiles:
		...
		if t.tile_owner != null and gc.board_state.has_monopoly(t.tile_owner, t_color): continue
		...
		if t.tile_owner != null and not t.tile_owner.is_bankrupt:
			...
			if t_color != "" and gc.board_state.has_monopoly(t.tile_owner, t_color):
				var monopoly_built := false
				for st in gc.tiles:
					if st.tile_type == BoardData.TileType.PROPERTY and st.tile_data.get("color","") == t_color:
						if st.funding > 0: monopoly_built = true; break
				if not monopoly_built:
					score += 800
			else:
				score += 150
	...
	if candidates.is_empty():
		print("[AI] No valid swap targets.")
		gc.show_default_actions()
		return
	...
	print("[AI] Swapping ", my_worst.tile_data.get("name","?"),
		  " ↔ ", best_target.tile_data.get("name","?"),
		  " (score: ", int(candidates[0]["score"]), ")")
...
func _ai_choose_richest_player(ai_player, money_swap: bool) -> int:
	var best_idx   = -1
	var best_score = -1.0
	...
	for i in range(gc.players.size()):
		...
		for t in p.properties:
			prop_value += t.tile_data.get("price", 0)
		var score: float
		if money_swap:
			score = float(p.money + prop_value/20.0)
		else: 
			score = float(p.money + prop_value)
		score += randf_range(0.0, 50.0)
