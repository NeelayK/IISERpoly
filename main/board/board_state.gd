...
func draw_chance_card():
	var card = BoardData.chance_cards.pick_random()
	return card

func draw_project_funds_card():
	var card = BoardData.fund_cards.pick_random()
	return card
...
func calculate_rent(tile):
	...
	if tile.tile_type == BoardData.TileType.PROPERTY:
		var funding = tile.funding # Assuming 0 means no buildings
